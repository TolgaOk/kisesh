"""No-UI Kitty kitten that reliably toggles restored Workbench layouts."""
...
if TYPE_CHECKING:
    ...
    from kitty_workbench.layout_toggle import LayoutBoss
...
class LayoutToggleHandler(Protocol):
    ...
    def __call__(self, boss: LayoutBoss) -> None:
        """Toggle the active Workbench tab's zoom layout."""


PROJECT_ROOT = Path.home() / ".local" / "lib" / "kitty-workbench"
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))
LAYOUT_TOGGLE_MODULE = reload(import_module("kitty_workbench.layout_toggle"))
